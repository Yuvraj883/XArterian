...
const URL = `https://x-arterian.vercel.app/api/products/${productId}`
...
fetch(URL)
  ...
  .then((data) => {
    ...
    productNode.querySelector('#product-brand').textContent =
    ...
    productNode.querySelector('#product-availability').textContent =
      productDetails?.availabilityStatus
    productNode.querySelector('#product-description').textContent =
      productDetails?.description
    productNode.querySelector(
      '#product-stock'
    ).textContent = `Only ${productDetails?.stock} left in stock`
    productNode.querySelector(
      '#product-reviews'
    ).textContent = ` ${productDetails?.reviews?.length} Reviews`
